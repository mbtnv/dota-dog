...
Долгоживущие сервисы `db`, `bot` и `worker` используют политику перезапуска
`unless-stopped`, поэтому после перезагрузки Docker или сервера база данных
поднимается вместе с приложением. Сервис `migrate` остаётся одноразовым и
запускается только во время `docker compose up`.
